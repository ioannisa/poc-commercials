...
/**
 * Android destination for server-rendered PDF bytes, all genuinely native OS
 * objects: SAF ACTION_CREATE_DOCUMENT to save, ACTION_VIEW to open, the
 * system share sheet (which carries Print) for printing. Requires the
 * FileProvider declared in the app manifest - without it ACTION_VIEW throws
 * FileUriExposedException.
 *
 * (Near-twin of the iOS sink on purpose: a shared android+ios source set
 * would gamble on the library's mobile metadata resolving in a custom group;
 * ~40 duplicated lines are cheaper than that build risk.)
 */
